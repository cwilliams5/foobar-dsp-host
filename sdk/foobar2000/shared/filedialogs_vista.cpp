...
#include <shobjidl.h>
#include <shtypes.h>
// RESONANCE PATCH: ATL (atlbase/atlcom) dropped so shared.dll builds without the VS "C++ ATL"
// component. The file already uses pfc::com_ptr_t everywhere; the only ATL use was two CComPtr in
// uEvalKnownFolder(), swapped to pfc::com_ptr_t below. Behavior is identical.
...
puGetOpenFileNameMultiResult SHARED_EXPORT uEvalKnownFolder(REFKNOWNFOLDERID id) {
	...
	try {
		pfc::com_ptr_t<IKnownFolderManager> mgr; pfc::com_ptr_t<IKnownFolder> folder; CoTaskMemObject<wchar_t*> path; // RESONANCE PATCH: pfc::com_ptr_t in place of ATL CComPtr
		EH << CoCreateInstance(__uuidof(KnownFolderManager), nullptr, CLSCTX_ALL, IID_IKnownFolderManager, (void**)mgr.receive_ptr());
		EH << mgr->GetFolder(id, folder.receive_ptr());
